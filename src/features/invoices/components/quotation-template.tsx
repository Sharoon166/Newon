...
export const QuotationTemplate = forwardRef<HTMLDivElement, QuotationTemplateProps>(
  ({ quotationData, onBack, onPrint, onSave }, ref) => {
    ...
    const additionalChargesTotal = quotationData.additionalCharges?.reduce((sum, charge) => sum + charge.value, 0) || 0;
    ...
    return (
      ...
      >
        ...
        {/* Items Table */}
        <div className="mb-4 print:mb-2 not-print:overflow-auto quotation-table-container">
          <table className="w-full border-collapse not-print:overflow-x-auto quotation-table">
            <thead>
              <tr className="bg-muted/50">
                <th className="p-3 print:py-2">#</th>
                <th className="p-3 print:py-2 max-w-xs">Description</th>
                <th className="p-3 print:py-2 text-right">Qty</th>
                <th className="p-3 print:py-2 text-right">Rate</th>
                <th className="p-3 print:py-2 text-right">Amount</th>
              </tr>
            </thead>
            <tbody>
              {quotationData.items.map((item, index) => (
                <tr key={item.id} className={index % 2 === 0 ? 'bg-white' : 'bg-muted/10'}>
                  <td className="p-3 print:py-2">{index + 1}.</td>
                  <td className="p-3 print:py-2 max-w-xs print:max-w-none">
                  ...
                  <td className="p-3 print:py-2 text-right">{item.quantity}</td>
                  <td className="p-3 print:py-2 text-right print:w-32 print:text-wrap">{formatCurrency(item.rate)}</td>
                  <td className="p-3 print:py-2 text-right print:w-32 print:text-wrap font-medium">{formatCurrency(item.amount)}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="flex justify-end mb-4 print:mb-2 print-no-break">
          <div className="w-full md:w-1/2">
            ...
            {quotationData.additionalCharges && quotationData.additionalCharges.length > 0 && (
              <>
