  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="text-sm text-muted-foreground whitespace-nowrap">
        {formatCurrency(row.original.purchasePrice || 0)}
      </div>
    ),
    minSize: 100,
    size: 120
  },
  {
    ...
    cell: ({ row }) => (
      <div className={`font-medium ${row.original.availableStock <= 0 ? 'text-destructive' : ''}`}>
        {formatCurrency(row.original.wholesalePrice || 0)}
      </div>
    ),
    minSize: 100,
    size: 100
          ...
          <PopoverContent className="w-64 p-2" align="start">
            <div className="space-y-2 divide-y-2 divide-muted">
              {inventory.map((item, index) => {
                ...
                return (
                  <div key={location.id} className="flex flex-col gap-1 text-sm not-last:pb-2">
                    <div className="font-medium">
                      {index + 1}. {location.name || 'Unknown Location'}
                    </div>
                    <div className="flex justify-between items-center">
                      {location.address && <span className="text-muted-foreground text-xs">{location.address}</span>}
                    </div>
