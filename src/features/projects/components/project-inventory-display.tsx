  ...
  if (invoices.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-sm text-muted-foreground mt-2">Link invoices to view their inventory items here</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
          ...
          <div key={invoice.invoiceNumber} className="space-y-3">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">{format(new Date(invoice.date), 'PPP')}</p>
              </div>
                      ...
                      <div className="ml-4 space-y-1">
                        {item.componentBreakdown?.map((comp, compIdx) => (
                          <p key={compIdx} className="text-sm text-muted-foreground">
                            • {comp.productName} ({comp.sku}) - Qty: {comp.quantity} @ {formatCurrency(comp.unitCost)} ={' '}
      ...
      <div className="pt-4 border-t">
        ...
        <div className="flex justify-between items-center mt-2">
          ...
          <p className="font-semibold">{formatCurrency(allItems.reduce((sum, item) => sum + item.totalPrice, 0))}</p>
        </div>
      </div>
    </div>
  );
