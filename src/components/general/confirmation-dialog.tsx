...
const ConfirmationDialog = ({
  ...
  const isConfirmDisabled = requireTextConfirmation ? confirmationText !== requireTextConfirmation : false;
  ...
  return (
    <Sheet open={open} onOpenChange={handleOpenChange}>
      <SheetContent side="bottom" className="sm:max-w-lg sm:rounded-lg mx-auto max-h-screen overflow-auto">
        {icon && <div className="flex justify-center py-4">{icon}</div>}
        <SheetHeader className="text-center m-0 py-0">
          <SheetTitle>{title}</SheetTitle>
          <SheetDescription
            className={cn({
              'text-left text-destructive bg-destructive/10 p-2 border-l-2 border-destructive':
                variant === 'destructive'
            })}
          >
            {description}
        ...
        <SheetFooter className="sm:justify-end">
          ...
          <Button variant="outline" onClick={handleCancel} disabled={isLoading || isProcessing} className="mr-2">
            {cancelText}
          </Button>
        </SheetFooter>
      </SheetContent>
